...
116+, desktop only). It opens a small always-on-top window with just the
stress ring, WPM, and backspace count — you can keep it visible over any
other app (a PDF, a video call, another tab) without switching back to
the full dashboard. It doesn't read keystrokes from other windows —
you still type into StudyPulse's own tab — it's a compact, glanceable
readout of the same live session. Click "Back to tab" (or close the
little window) to bring the full widget back into the dashboard.
